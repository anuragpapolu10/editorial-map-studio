...
### Data Tab (Phase 1 — Points, Bubbles, Spikes & Heatmaps)
...
- [x] **Spike visualization** — vertical spike/bar markers sized by value column. Two styles: `pointed` (triangular spikes) and `bar` (flat-topped bars). Configurable height (20–300px), width (2–20px), color (custom color picker), and opacity. Spikes are rendered as MapLibre fill polygons with outline strokes, recomputed on `moveend` using screen-to-geographic coordinate conversion (`map.project()` / `map.unproject()`). Translucent fill with opaque stroke for clarity.
- [x] **Spike labels & values** — separate toggle for values at spike tips and city name labels at spike bases. Values use `text-anchor: 'bottom'` at tip coordinates, labels use `text-anchor: 'top'` at base coordinates. Shared text size slider (8–20px), commas toggle, and append text input for suffix units. Three separate GeoJSON sources/layers: spike polygons, tip labels, base labels — all recomputed together on `moveend`.
- [x] **Labels & values (points/bubbles)** — split into two separate MapLibre symbol layers: value layer above (`text-anchor: 'bottom'`) and label layer below (`text-anchor: 'top'`). Consistent positioning across all viz types: values above, labels below. Layers are scoped to points/bubbles only — automatically removed when switching to spikes or heatmap.
...
Key files:
- `src/dataStore.ts` — DataState interface (includes spike fields: `spikeStyle`, `spikeHeight`, `spikeWidth`, `spikeColor`, `spikeShowValues`, `spikeCommas`, `spikeSuffix`, `spikeShowLabels`, `spikeLabelSize`), CSV parsing, column auto-detection, `pointsToGeoJSON()`, `extractPoints()`, geocoding (`geocodeLocations()`, Nominatim API). Also exports `HeatmapLegendInfo` interface for legend data flow.
- `src/components/DataTools.tsx` — Data tab UI, MapLibre layer management (circle + heatmap + spike + symbol layers), geocode flow. Contains `HEATMAP_RAMPS` color definitions, `HEATMAP_LAYER_ID`, spike layer IDs (`SPIKE_SOURCE_ID`, `SPIKE_LAYER_ID`, `SPIKE_LABELS_SOURCE_ID`, `SPIKE_BASE_LABELS_SOURCE_ID`), and separate value/label layer IDs (`VALUE_LAYER_ID`, `LABEL_LAYER_ID`). `buildSpikeGeoJSON()` returns three feature collections: spike polygons, tip labels, and base labels. Reports legend info via `onHeatmapLegend` callback.
